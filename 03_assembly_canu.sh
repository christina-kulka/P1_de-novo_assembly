...
CLEAN_READS="${PREPROC_OUTPUT_DIR}/${SAMPLE}/${SAMPLE}_final.fastq"
...
echo "Output directory: ${ASSEMBLY_OUTPUT_DIR}/${SAMPLE}/canu_out"
...
canu \
    -p "$SAMPLE" \
    -d "${ASSEMBLY_OUTPUT_DIR}/${SAMPLE}/canu_out" \
...
if [ -f "${ASSEMBLY_OUTPUT_DIR}/${SAMPLE}/canu_out/${SAMPLE}.contigs.fasta" ]; then
    echo "Canu assembly completed successfully!"
    grep "^>" "${ASSEMBLY_OUTPUT_DIR}/${SAMPLE}/canu_out/${SAMPLE}.contigs.fasta" | wc -l | xargs echo "Number of contigs:"
    echo "Assembly file: ${ASSEMBLY_OUTPUT_DIR}/${SAMPLE}/canu_out/${SAMPLE}.contigs.fasta"
    
    # Also check for unitigs (uncorrected assembly)
    if [ -f "${ASSEMBLY_OUTPUT_DIR}/${SAMPLE}/canu_out/${SAMPLE}.unitigs.fasta" ]; then
        echo "Unitigs file: ${ASSEMBLY_OUTPUT_DIR}/${SAMPLE}/canu_out/${SAMPLE}.unitigs.fasta"
    fi
else
